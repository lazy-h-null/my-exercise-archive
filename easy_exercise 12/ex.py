...
class Restaurant(Place):
    ...
    def get_popup_text(self):
        return f"<b>🍽️RESTAURANT: {self.name}</b><br>Food: {self.food_type}"
...
class Park(Place):
    ...
    def get_popup_text(self):
        ...
        return f"<b>🏞️PARK: {self.name}</b><br>Playground: {status}"
...
class Museum(Place):
    ...
    def get_popup_text(self):
        return f"<b>🏛️MUSEUM: {self.name}</b><br>Entry: €{self.entry_fee}"
...
class Cafe(Restaurant):
    ...
    def get_popup_text(self):
        wifi_icon = "✅ WiFi" if self.has_wifi else "❌ No WiFi"
        return f"<b>☕️CAFE: {self.name}</b><br>Food: {self.food_type}<br>{wifi_icon}"
...
class MyMap:
    ...
    def find_closest_places(self):
        """Find the two closest places on my map"""
        if len(self.places) < 2:
            return None
...
def create_my_places():
    ...
    # restaurants = [...]
    places.append(Restaurant("Le Comptoir de l'Arc", 48.87114771035836, 2.299107952286957, "French"))
    places.append(Restaurant("Le P'tit Lecourbe", 48.84456243024377, 2.3089394588882945, "French"))
    # TODO: Add at least 2 parks
    # parks = [...]
    places.append(Park("Jardin des Tuileries", 48.863625673561536, 2.3276659583054626, True))
    places.append(Park("Champ-de-Mars", 48.85696237988104, 2.2990632430124265, True))
    # TODO: Add at least 1 museum
    # museums = [...]
    places.append(Museum("Château de Versailles", 48.806843420949974, 2.119717110888288, 21))
    # Combine all places
    # places.extend(restaurants)
    # places.extend(parks)
    # places.extend(museums)
    places.append(Cafe("Café de Flore",48.85817650318787, 2.3306489366321026, "Cafe", True))

    # places.append(Restaurant("Gwangjang Market", 37.5701, 126.9992, "Korean Street Food"))
    # places.append(Park("Olympic Park", 37.5208, 127.1215, True))
    # places.append(Museum("Changdeokgung Palace", 37.5794, 126.9910, 3))
    # places.append(Cafe("Coffee Hanyakbang", 37.5663, 126.9912, "Coffee & Dessert", True))
...
def main():
    ...
    print(f"\n📝 Adding {len(my_places)} places to the map...")
    for p in my_places:
        mymap.add_place(p)
    ...
    result = mymap.find_closest_places()
    if result is not None:
        closest_pair, distance = result
        if closest_pair is not None:
            place1, place2 = closest_pair

        print(f"The two closest places are: {place1.name} and {place2.name}")
        print(f"Distance between them: {distance} km")
        print("-" * 31)
    else:
        print("\n📍 Not enough places to find the closest pair.")
